...
def main():
    ...
    train_loader = DataLoader(
        train, batch_size=args.batch_size, shuffle=True, num_workers=args.n_worker
    )
    test_loader = DataLoader(
        test, batch_size=args.batch_size, shuffle=False, num_workers=args.n_worker
    )
            ...
            if b % 10000 == 0:
                ...
                if not best_loss or loss < best_loss:
                    ...
                    if not os.path.isdir(".save"):
                        os.makedirs(".save")
                    torch.save(
                        model.state_dict(), "./.save/trfm_new_%d_%d.pkl" % (e, b)
                    )
                    best_loss = loss
