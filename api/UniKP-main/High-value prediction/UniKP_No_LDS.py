...
def Seq_to_vec(Sequence):
    ...
    tokenizer = T5Tokenizer.from_pretrained(
        "/home/yuhan/PepTrans/prot_t5_xl_uniref50", do_lower_case=False
    )
    model = T5EncoderModel.from_pretrained("/home/yuhan/PepTrans/prot_t5_xl_uniref50")
    gc.collect()
...
def Smooth_Label(Label_new):
    ...
    eff_label_dist = convolve1d(
        np.array(emp_label_dist[:-10]), weights=lds_kernel_window, mode="constant"
    )
    # plt.figure(figsize=(6,4))
