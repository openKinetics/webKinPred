...
class RNNSeq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):  # (T,B)
        ...
        outputs = Variable(
            torch.zeros(max_len, batch_size, vocab_size)
        ).cuda()  # (T,B,V)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--n_epoch", "-e", type=int, default=20, help="number of epochs"
    )
    parser.add_argument(
        "--vocab", "-v", type=str, default="data/vocab.pkl", help="vocabulary (.pkl)"
    )
    parser.add_argument(
        "--train_data",
        type=str,
        default="data/chembl24_bert_train.csv",
        help="train corpus (.csv)",
    )
    parser.add_argument(
        "--test_data",
        type=str,
        default="data/chembl24_bert_test.csv",
        help="test corpus (.csv)",
    )
    parser.add_argument(
        "--out-dir", "-o", type=str, default="../result", help="output directory"
    )
    parser.add_argument("--name", "-n", type=str, default="ST", help="model name")
    parser.add_argument(
        "--seq_len", type=int, default=220, help="maximum length of the paired seqence"
    )
    parser.add_argument("--batch_size", "-b", type=int, default=16, help="batch size")
    parser.add_argument(
        "--n_worker", "-w", type=int, default=16, help="number of workers"
    )
    parser.add_argument(
        "--hidden", type=int, default=256, help="length of hidden vector"
    )
    ...
    parser.add_argument(
        "--lr-decay", type=int, default=50000, help="lr decay step size"
    )
    parser.add_argument("--log-freq", type=int, default=100, help="log frequency")
    parser.add_argument(
        "--gpu", metavar="N", type=int, nargs="+", help="list of GPU IDs to use"
    )
    parser.add_argument(
        "--checkpoint", "-c", type=str, default=None, help="Parameter to load"
    )
    parser.add_argument(
...
def main():
    ...
    for e in range(1, args.n_epoch):
        for b, data in tqdm(enumerate(train_loader)):
            ...
            if b % 100 == 0:
                print(
                    "Train {:3d}: iter {:5d} | loss {:.3f} | ppl {:.3f}".format(
                        e, b, loss.item(), math.exp(loss.item())
                    )
                )
            if b % 1000 == 0:
