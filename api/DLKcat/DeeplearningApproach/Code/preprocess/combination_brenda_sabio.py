...
with open(
    "../../Data/database/Kcat_sabio_clean_unisubstrate_2.tsv", "r", encoding="utf-8"
) as file1:
...
for line in brenda_lines:
    ...
    if smiles is not None:
        ...
        Kcat_data_include_value.append(
            [ECNumber, Substrate.lower(), EnzymeType, Organism, Value, Unit]
        )
...
print(
    len(new_lines)
)  # 48659 included all elements, 46165 included all except for Kcat value and unit,  44166 substrate lower and upper
...
for new_line in new_lines:
    ...
    max_value = max(
        Kcat_values
    )  # choose the maximum one for duplication Kcat value under the same entry as the data what we use
...
with open(
    "../../Data/database/Kcat_combination_0730.tsv", "r", encoding="utf-8"
) as infile:
...
for line in Kcat_lines:
    ...
    Kcat_data_include_value.append(
        [ECNumber, EnzymeType, Organism, Smiles, Value, Unit]
    )
...
print(
    len(new_lines)
)  # 43495 included all elements, 41558 included all except for Kcat value and unit
