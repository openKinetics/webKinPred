...
def uniprotID_entry():
    ...
    with open(
        "../../Data/database/Kcat_combination_0731.tsv", "r", encoding="utf-8"
    ) as file:
...
def seq_by_ec_organism(ec, organism):
    ...
    params = {
        "query": "ec:%s AND organism:%s AND reviewed:yes" % (ec, organism),
        "format": "fasta",
    }
...
def combine_sequence():
    ...
    with open(
        "../../Data/database/nouniprotID_entry_all.json", "r"
    ) as file2:  # By Uniprot API
        nouniprot_file2 = json.load(file2)

    with open(
        "../../Data/database/nouniprotID_entry_brenda.json", "r"
    ) as file3:  # By BRENDA API
    ...
    for line in Kcat_lines:
        ...
        try:  # local variable 'RetrievedSeq' referenced before assignment
            if (
                len(RetrievedSeq) == 1 and EnzymeType == "wildtype"
            ):  # 21108 for all, 9529 wildtype, 11579 mutant (EnzymeType != 'wildtype')
            ...
            if len(RetrievedSeq) == 1 and EnzymeType != "wildtype":
                ...
                if len(mutant1[0]) != len(mutant2):
                    ...
                    print(
                        str(n) + "---------------------------"
                    )  # some are mapped, some are not mapped. R234G/R234K (60, 43 mapped, 17 not mapped)
    ...
    with open(
        "../../Data/database/Kcat_combination_0918_wildtype_mutant.json", "w"
    ) as outfile:
        json.dump(entries, outfile, indent=4)
