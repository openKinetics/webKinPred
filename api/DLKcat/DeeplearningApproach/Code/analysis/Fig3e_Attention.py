...
fingerprint_dict = subsequence_model.load_pickle(
    "../../Data/input/fingerprint_dict.pickle"
)
...
def plot_attention_weights(
    attention_profiles,
    wildtype_like_positions,
    wildtype_decreased_positions,
    wildtype_like,
    wildtype_decreased,
):
    positions = list()
    ...
    plt.plot(
        positions, weights, color="k", linestyle="--", linewidth=0.75
    )  # color='k'  color='#A65628'
    ...
    plt.scatter(
        ...
        wildtype_like,
        s=[
            Counter(wildtype_like_positions)[position] * 5
            for position in wildtype_like_positions
        ],
        color="#2166ac",
        marker="o",
        label="Wildtype_like",
    )  # marker='o' or marker='s'
    sc = plt.scatter(
        wildtype_decreased_positions,
        wildtype_decreased,
        s=[
            Counter(wildtype_decreased_positions)[position] * 5
            for position in wildtype_decreased_positions
        ],
        color="#b2182b",
        marker="o",
        label="Wildtype_decreased",
    )
...
def extract_wildtype_attention(wildtype_entry):
    ...
    Kcat_model = subsequence_model.KcatPrediction(
        device,
        n_fingerprint,
        n_word,
        2 * dim,
...
def output_wildtype_enzyme(wildtype_attention_profiles, sequence):
    ...
    with open(
        "../../Results/output/supple_wildtype_PNP_attention_weights.tsv", "w"
    ) as outfile:
        i = 0
            ...
            for data in Kcat_data:
                ...
                if entry == wildtype_mutant_entry[0]:
                    ...
                    if "." not in smiles and float(Kcat) > 0:
                        ...
                        different_positions = compare_mutant_wildtype_sequence(
                        ...
                        if normalized_value >= 0.5 and normalized_value < 2.0:
                            ...
                            for position in different_positions:
                                ...
                                wildtype_like.append(
                                    float(wildtype_attention_profiles[position])
                                )
                                alldata["type"].append("Wildtype_like")
                                alldata["entry"].append(entry_name)
                                alldata["weights"].append(
                                    float(wildtype_attention_profiles[position])
                                )
