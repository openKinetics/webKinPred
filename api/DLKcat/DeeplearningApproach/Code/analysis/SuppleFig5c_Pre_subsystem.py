    ...
    for data in datasets:
        ...
        try:
            if EC_Kcat[EC_Number] and float(data["Value"]) > 0:
                ...
                EC_Kcat[EC_Number].append(
                    value
                )  # math.log10(float(data['Value']))   float(data['Value'])
        except:
...
def plot_subsystem_Kcat_counts():
    ...
    plt.savefig(
        "../../Results/figures/subsystem_Kcat_counts_4.pdf",
        dpi=400,
        bbox_inches="tight",
    )
...
def plot_subsystem_distribution():
    ...
    for types, Kcat in types_Kcat.items():
        if types in ["Primary-CE", "Intermediate", "Primary-AFN", "Secondary"]:
            ...
            print(
                "The median value of %s is %.2f" % (types, math.pow(10, median(Kcat)))
            )
