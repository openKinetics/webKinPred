...
def plot_attention_weights(attention_profiles):
    ...
    plt.savefig(
        "../../Results/figures/attention_weights_case.pdf", dpi=400, bbox_inches="tight"
    )
...
def main():
                    ...
                    if "." not in smiles and float(Kcat) > 0:
                        ...
                        if normalized_value >= 0.5 and normalized_value < 2.0:
                            ...
                            for position in different_positions:
                                ...
                                wildtype_like.append(
                                    float(wildtype_attention_profiles[position - 1])
                                )
                                alldata["type"].append("Wildtype_like")
                                alldata["entry"].append(entry_name)
                                alldata["weights"].append(
    ...
    ax = sns.boxplot(
        ...
        x="type",
        y="weights",
        order=["Wildtype_like", "Wildtype_decreased"],
        palette=palette,
        showfliers=False,
        linewidth=0.5,
        width=0.5,
