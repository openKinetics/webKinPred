...
class KcatPrediction(nn.Module):
    ...
    def attention_cnn(self, x, xs, layer):
        ...
        # print(max_attention)
        attention_profiles = [
            "%.4f" % (float(attention) / max_attention)
            for attention in attention_weights
        ]

        return torch.unsqueeze(torch.mean(ys, 0), 0), attention_profiles
    ...
    def forward(self, inputs):
        ...
        protein_vector, attention_profiles = self.attention_cnn(
            compound_vector, word_vectors, layer_cnn
        )
        # print(protein_vector)
        # print('The length of protein vectors is:', len(protein_vector[0]))
        ...
        return interaction, attention_profiles
...
if __name__ == "__main__":
    ...
    (dim, layer_gnn, window, layer_cnn, layer_output, decay_interval, iteration) = map(
        int,
