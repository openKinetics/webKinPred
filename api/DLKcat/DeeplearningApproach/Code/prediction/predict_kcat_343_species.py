...
def main():
    ...
    Kcat_model.load_state_dict(
        torch.load(
            "../../Results/output/all--radius2--ngram3--dim20--layer_gnn3--window11--layer_cnn3--layer_output3--lr1e-3--lr_decay0.5--decay_interval10--weight_decay1e-6--iteration30",
            map_location=device,
        )
    )
    ...
    for organism in organisms:
        ...
        with open(
            "../../Data/input/kcatpredictionfile/%sForKcatPrediction.txt" % organism,
            "r",
        ) as infile:
        ...
        file = open(
            "../../Results/output/343species/%s_PredictionResults.txt" % organism, "w"
        )
                    ...
                    for sequence_id in sequence_info:
                        if sequence_id:
                            ...
                            if organism != "Saccharomyces_cerevisiae":
                                ...
                                sequence = proteinSeq[
                                    "Saccharomyces_cerevisiae@" + sequence_id
                                ]
                            # print(smiles)
                            if "." not in smiles:
                                # i += 1
