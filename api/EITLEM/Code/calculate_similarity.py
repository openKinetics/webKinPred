...
def getScoreA(arg):
    ...
    for i in searchSeq:
        alignments = align.align_optimal(
            q,
            seq.ProteinSequence(seqIndex[i]),
            matrix,
            local=False,
            gap_penalty=(-10, -0.5),
        )
...
def getScoreB(arg):
    ...
    for item in searchSeq:
        alignments = align.align_optimal(
            t,
            seq.ProteinSequence(re.sub(r"[UZOB]", "X", item)),
            matrix,
            local=False,
            gap_penalty=(-10, -0.5),
        )
...
def seq_smi(args):
    if args.Type == 0:
        ...
        if args.loadType is not None:
            queryPairinfo = torch.load(
                f"../Data/{args.loadType}/{args.loadType}TestPairInfo"
            )
            searchPairInfo = torch.load(
                f"../Data/{args.loadType}/{args.loadType}TrainPairInfo"
            )
