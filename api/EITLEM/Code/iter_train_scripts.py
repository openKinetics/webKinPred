...
def kineticsTrainer(kkmPath, TrainType, Type, Iteration, log10, molType, device):
    ...
    if kkmPath is not None:
        ...
        if Type == "KCAT":
            ...
            pretrained_para = {
                ...
                if "kcat" in k and k[5:] in weights
            }
    ...
    if kkmPath is not None:
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[50, 80], gamma=0.9
        )
    ...
    for epoch in range(1, Epoch + 1):
        train_MAE, train_rmse, train_r2, loss_train = trainer.run(
            ...
            optimizer,
            len(train_pair_info),
            f"{Iteration}iter epoch {epoch} train:",
        )
        MAE_dev, RMSE_dev, R2_dev, loss_dev = tester.test(
            model,
            valid_loader,
            len(test_pair_info),
            desc=f"{Iteration}iter epoch {epoch} valid:",
        )
        scheduler.step()
        writer.add_scalars(
            "loss", {"train_loss": loss_train, "dev_loss": loss_dev}, epoch
        )
        writer.add_scalars(
            "RMSE", {"train_RMSE": train_rmse, "dev_RMSE": RMSE_dev}, epoch
        )
        ...
        tester.save_model(
            model,
            file_model
            + f"{molType}_trainR2_{train_r2:.4f}_devR2_{R2_dev:.4f}_RMSE_{RMSE_dev:.4f}_MAE_{MAE_dev:.4f}",
        )  # 保存
    pass
...
def KKMTrainer(kcatPath, kmPath, TrainType, Iteration, log10, molType, device):
    ...
    pretrained_kcat_para = {
        k: v for k, v in kcat_pretrained.items() if k in kcat_parameters
    }
    ...
    for epoch in range(1, Epoch + 1):
        ...
        tester.save_model(
            model,
            file_model
            + f"{molType}_trainR2:{train_r2:.4f}_devR2_{R2_dev:.4f}_RMSE_{RMSE_dev:.4f}_MAE_{MAE_dev:.4f}",
        )
...
def TransferLearing(
    Iterations, TrainType, log10=False, molType="MACCSKeys", device=None
):
    for iteration in range(1, Iterations + 1):
        if iteration == 1:
            ...
            kineticsTrainer(
                kkmPath, TrainType, "KCAT", iteration, log10, molType, device
            )
...
def parse_args():
    ...
    parser.add_argument(
        "-m", "--molType", type=str, required=False, default="MACCSKeys"
    )
