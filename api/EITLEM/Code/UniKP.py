...
def Seq_to_vec(Sequence):
    ...
    sequences_Example = [
        " ".join(list(re.sub(r"[UZOB]", "X", sequence))) for sequence in Sequence
    ]
    tokenizer = T5Tokenizer.from_pretrained(
        "../Weights/UniKP/prot_t5_xl_uniref50", do_lower_case=False
    )
    ...
    for i in trange(n_iter + 1, desc="Extract seq embeding"):
        ...
        if len(sequence_batch) > 0:
            ids = tokenizer.batch_encode_plus(
                sequence_batch, add_special_tokens=True, padding=True
            )
...
class UniKp_predictor:
    ...
    def fit(self, train_pair_info, all_pair, index_seq, index_smiles, Type):
        ...
        if os.path.exists(f"../Weights/UniKP/{Type}_prot_t5_embeding.pt"):
            ...
            torch.save(
                {"embeding": self.seqs_feature, "mapping": self.seq_mapping},
                f"../Weights/UniKP/{self.Type}_prot_t5_embeding.pt",
            )

        x_train, y_train = self._load_dataset(
            train_pair_info, self.index_seq, self.index_smiles
        )
        self.model = ExtraTreesRegressor(n_jobs=128)
        self.model.fit(x_train, y_train)

    def test(self, test_pair, ret=False):
        x_test, y_test = self._load_dataset(
            test_pair, self.index_seq, self.index_smiles
        )
