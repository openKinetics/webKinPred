...
def reaction_preprocessing(substrate_list, product_list):
    ...
    df_reaction = pd.DataFrame(
        data={"substrates": substrate_list, "products": product_list}
    )
    ...
    for ind in df_reaction.index:
        ...
        if pd.isnull(left_site) or pd.isnull(right_site):
            ...
            difference_fp = Chem.rdChemReactions.CreateDifferenceFingerprintForReaction(
                rxn_forward
            )
    ...
    for met in metabolites:
        ...
        if met_type == "KEGG":
            Smarts = Chem.MolToSmarts(
                Chem.MolFromMolFile(
                    join(CURRENT_DIR, "data", "mol-files", met + ".mol")
                )
            )
        elif met_type == "InChI":
...
def is_SMILES(met):
    ...
    if m is None:
        ...
        try:
            ...
            print(
                '.......Metabolite string "%s" is in SMILES format but has invalid chemistry'
                % met
            )
            return False
    return True
...
def is_InChI(met):
    ...
    if m is None:
        ...
        try:
            ...
            print(
                '.......Metabolite string "%s" is in InChI format but has invalid chemistry'
                % met
            )
            return False
    return True
