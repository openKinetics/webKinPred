...
def create_mmseqs_database(fasta_file_path: str, session_id: str) -> Tuple[str, str]:
    ...
    run_and_stream(_mmseqs_cmd("createdb", fasta_file_path, query_db), session_id=session_id)
...
def parse_mmseqs_results(
    result_file: str, query_file_path: str
...
def calculate_identity_histogram(
    ...
    histogram_percentages = {
        k: (v / total_seqs * 100) if total_seqs else 0.0 for k, v in histogram_counts.items()
    }
